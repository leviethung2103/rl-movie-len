...
# from stable_baselines.common.base_class import ActorCriticRLModels
...
def import_data_for_env() -> Dict[str, pd.DataFrame]:
    ...
    """
    download_data()
    kwargs = dict([(label, data) for label, data in zip(["data", "item", "user"], import_data())])
    return kwargs


def evaluate(model: BaseAlgorithm, env: RecoEnv, num_steps: int = 1000) -> None:
