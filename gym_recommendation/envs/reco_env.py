...
class RecoEnv(Env):
    ...
    def __init__(self, data: pd.DataFrame, item: pd.DataFrame, user: pd.DataFrame, seed: int = 1):
        ...
        self._seed = seed
        self._random_state = np.random.default_rng(seed=self._seed)
    ...
    def reset(self, seed=None, options=None) -> np.ndarray:
        ...
        self.done = False
        print(
            f"Reco is being reset() --> "
            f"first step = {self.local_step_number} | "
            f"Total_correct = {self.total_correct_predictions}"
        )
        self.total_correct_predictions = 0
        obs = self._get_observation(step_number=self.local_step_number)
        infos = {}
        return obs, infos
    ...
    def _get_movie_genre(item: pd.DataFrame) -> Dict[int, np.ndarray]:
        ...
        """
        movie_genre = dict([(movie_id, np.empty(19, dtype=np.float32)) for movie_id in item["movie_id"].tolist()])
    ...
    def _get_observation(self, step_number: int = 0) -> np.ndarray:
        ...
        return np.concatenate((user_mean, movie_mean, movie_genre_bucket, age_bucket, occupation_bucket, gender_bucket))
    ...
    def _get_reward(self, action: int, step_number: int) -> float:
        ...
        if prediction_difference == 0:
            ...
            reward += np.log(1.0 - prediction_difference / 5)
        return reward
